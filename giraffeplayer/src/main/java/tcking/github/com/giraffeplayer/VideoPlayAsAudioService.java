...
public class VideoPlayAsAudioService extends Service {
    ...
    public void onCreate() {
        ...
        this.videoList = SharedPref.getVideoList(getApplicationContext());
        createChannel();
    ...
    public void videoPlay() {
        try {
            ...
            this.mediaPlayer.setDataSource(String.valueOf(Uri.parse(videoList.get(videoPosition).getDATA())));
            this.mediaPlayer.setAudioStreamType(3);
            this.mediaPlayer.prepareAsync();
            this.mediaPlayer.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
                public void onPrepared(MediaPlayer mediaPlayer) {
                    mediaPlayer.seekTo(SharedPref.getInt(getApplicationContext(), SharedPref.PREF_CURRENT_SEEK_POSITION));
                    mediaPlayer.start();
    ...
    public void handleCommandIntent(Intent intent) {
        ...
        if (NEXT_ACTION.equals(action)) {
            if (videoPosition != this.videoList.size() - 1) {
                videoPosition++;
                this.isRefresh = true;
                this.handler.removeCallbacks(this.runnable);
                onButtonEvent();
    ...
    private Notification buildNotification() {
        ...
        String name = this.videoList.get(videoPosition).getDISPLAY_NAME();
        boolean isPlaying = this.mediaPlayer.isPlaying();
        String str = VideoPlayerUtils.makeShortTimeString(this, (long) (this.mediaPlayer.getCurrentPosition() / 1000))
                + " / " + VideoPlayerUtils.makeShortTimeString(this, (long) (Integer.parseInt(videoList.get(videoPosition).getDURATION()) / 1000));
        int i = isPlaying ? R.drawable.ic_pause : R.drawable.ic_play;


        @SuppressLint("WrongConstant") PendingIntent activity = PendingIntent.getActivity(this, 0, new Intent(GiraffePlayerActivity
                .getIntent(this, videoList, videoPosition, mediaPlayer.getCurrentPosition())), 134217728);

        File file = new File(this.videoList.get(videoPosition).getDATA());
        ...
        this.notificationBuilder.setSmallIcon(R.drawable.ic_thumbnail_notificaiton).setContentIntent(activity).setCustomContentView(remoteViews).setCustomBigContentView(remoteViews2);
        if (isPlaying) {
    ...
    public void setSongsCurrentValues() {
        ...
        SharedPref.save(this, SharedPref.PREF_CURRENT_SEEK_POSITION, mediaPlayer.getCurrentPosition());
